...
if (!process.env.SENDGRID_BROADCAST_API_KEY) {
  throw new Error('SENDGRID_API_KEY is missing in .env');
}

sgMail.setApiKey(process.env.SENDGRID_BROADCAST_API_KEY); // Changed to transactional key
...
console.log('SendGrid initialized with:', {
  ...
  keyPrefix: process.env.SENDGRID_BROADCAST_API_KEY?.substring(0, 6) + '...'
});
